...
def configure_vxlan(master_node, master_ip, worker_nodes, ovs_bridge):
    ...
    for worker_name, worker_ip in worker_nodes.items():
        print(f"*" * 50)
        print(f"Configuring VXLAN tunnel between master node and worker node {worker_name} ({worker_ip})")
        ...
        for ovs_bridge, vxlan_key in vxlan_keys.items():
            ...
            print(f"Adding VXLAN port ({master_node}): {vxlan_port} to OVS bridge: {ovs_bridge}")
            cmd = (
                f"ovs-vsctl add-port {ovs_bridge} {vxlan_port} "
                f"-- set interface {vxlan_port} type=vxlan options:remote_ip={worker_ip} options:key={vxlan_key}"
            )
            output, error = ssh_run_sudo_command(master_ip, 22, username, password, cmd)
            if error:
                print(f"Error configuring VXLAN on master node: {error}")

            cmd = f"ovs-vsctl del-port {ovs_bridge} {vxlan_port} 2>/dev/null || true"
            output, error = ssh_run_sudo_command(worker_ip, 22, username, password, cmd)

            print(f"Adding VXLAN port ({worker_name}): {vxlan_port} to OVS bridge: {ovs_bridge}")
            cmd = (
                f"ovs-vsctl add-port {ovs_bridge} {vxlan_port} "
                f"-- set interface {vxlan_port} type=vxlan options:remote_ip={master_ip} options:key={vxlan_key}"
            )
            output, error = ssh_run_sudo_command(worker_ip, 22, username, password, cmd)
            if error:
                print(f"Error configuring VXLAN on worker node: {error}")
...
if __name__ == "__main__":
    ...
    configure_vxlan(master_node, master_ip, nodes, args.ovs_bridges)
